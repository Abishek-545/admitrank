# app.py
# AdmitRank — Train • Predict • Explain (final, Top-K strictly from prediction set, safe histograms)
...
def split_train_eval(
    ...
    # Optional label encoding for non-numeric classes
    le = None
    y_enc = y.copy()
    if task == "classification" and y_enc.dtype.kind not in "ifb":
        le = LabelEncoder()
    ...
    # Simple permutation-based per-original-column importance
    imp_df = pd.DataFrame()
    try:
        agg = []
        base_score = pipe.score(X_te, y_te)
        for col in X.columns:
            X_perm = X_te.copy()
            X_perm[col] = np.random.permutation(X_perm[col].values)
            s = pipe.score(X_perm, y_te)
            agg.append((col, base_score - s))
        imp_df = pd.DataFrame(agg, columns=["feature", "importance"]).sort_values(
...
def ensure_model() -> bool:
    return all(k in st.session_state for k in ("model", "fitted"))


def extract_id_from_name(name: str) -> Optional[str]:
    """Match <id>_<TYPE>.[pdf] — extension optional."""
...
def simple_doc_score(text: str) -> float:
    ...
    return float(np.clip(0.6 * length_score + 0.4 * richness, 0, 1))
...
def proba_from_model(model_pack: dict, df_pred: pd.DataFrame) -> np.ndarray:
    """Return [0,1] probabilities strictly for the prediction set."""
...
def safe_hist(series: pd.Series, bins: int = 20, range_=(0.0, 1.0), title="Histogram"):
    """Streamlit-safe histogram using NumPy + bar_chart."""
    arr = pd.to_numeric(series, errors="coerce").dropna().values
    if arr.size == 0:
        st.info("No numeric data to plot.")
        return
    hist, edges = np.histogram(arr, bins=bins, range=range_)
    centers = (edges[:-1] + edges[1:]) / 2
    dfh = pd.DataFrame({"count": hist}, index=pd.Index(centers, name="value"))
    st.bar_chart(dfh, use_container_width=True)
...
st.markdown(
    ...
    <p style="color:#aaa">
      ...
      (extension optional). Missing PDFs are OK — the system will simply use the tabular model.
    </p>
    """,
...
with tab_train:
    ...
    if hist_upload is not None:
        ...
        if df_hist.empty:
            ...
            features = st.multiselect("Feature columns", options=cols, default=feature_default)
            ...
            if st.button("Train", type="primary"):
                if not features:
                    ...
                    st.success("Model trained and stored in session. Switch to the Predict tab next.")

                    st.markdown("### Validation metrics")
                    mc = st.columns(len(metrics) or 1)
                    for (k, v), col in zip(metrics.items(), mc):
                        with col:
                            st.metric(k, f"{v:.4f}")
                    ...
                    if infer_task(df_hist[target]) == "classification":
                        ...
                        safe_hist(df_hist[target], bins=30, range_=(float(df_hist[target].min()), float(df_hist[target].max())))

                    if not imp_df.empty:
                        st.markdown("### Feature importance (permutation)")
                        st.dataframe(imp_df, use_container_width=True)
                        st.bar_chart(imp_df.set_index("feature").head(15), use_container_width=True)
...
with tab_predict:
    ...
    if not ensure_model():
        ...
        st.markdown(f"**Model expects features:** `{', '.join(features)}`")

        pred_upload = st.file_uploader("Upload new applicants CSV", type=["csv", "xlsx", "xls"], key="pred_csv")
        colA, colB = st.columns([2, 1])
        with colA:
            zip_upload = st.file_uploader(
                "Optional: ZIP with SOP/LOR/CV PDFs (named like 1234_SOP.pdf, 1234_LOR1.pdf, 1234_CV.pdf)",
                type=["zip"],
                key="docs_zip",
            )
        with colB:
        ...
        if not df_pred.empty:
            ...
            id_col = st.selectbox("Key column for matching docs", options=id_candidates if id_candidates else list(df_pred.columns))

            group_options = [""] + list(df_pred.columns)
            group_pick = st.selectbox("Optional: Top-K per group (leave blank for overall)", options=group_options)
            group_col = None if group_pick == "" else group_pick
            k_value = st.number_input("K (overall or per group)", 1, max(1, len(df_pred)), 5)

            # --- PROBABILITIES (Prediction set only) ---
            try:
                p_tabular = proba_from_model(model_pack, df_pred)
            except Exception as e:
                st.error(f"Could not score the prediction data. {e}")
                st.stop()

            # Doc index
            doc_index = build_doc_index_from_zip(zip_upload) if zip_upload is not None else {}

            # Align doc scores to prediction rows (missing -> NaN)
            ids_as_str = df_pred[id_col].astype(str).fillna("")
            doc_scores = np.array([doc_index.get(_id, np.nan) for _id in ids_as_str])

            # Fusion: if doc NaN -> fall back to tabular only for that row
            ds = np.where(np.isfinite(doc_scores), doc_scores, p_tabular)
            p_final = alpha * p_tabular + (1 - alpha) * ds

            # --- Build output strictly from prediction set ---
            out = df_pred.copy()
            ...
            safe_hist(out["p_final"], bins=20, range_=(0.0, 1.0))

            st.markdown("### Top-K applicants (from prediction set only)")
            if group_col is None:
                # Overall Top-K strictly from df_pred
                topk = out.sort_values("p_final", ascending=False).head(int(k_value))
                st.dataframe(topk, use_container_width=True)
            else:
                # Per-group Top-K strictly from df_pred
                def take_topk(g):
                    return g.sort_values("p_final", ascending=False).head(int(k_value))

                topk = out.groupby(group_col, group_keys=False).apply(take_topk)
                st.dataframe(topk, use_container_width=True)

            st.markdown("### Optional: Mean p_final by group")
            if group_col is None:
                st.write(f"Mean p_final (overall): **{out['p_final'].mean():.4f}**")
            else:
...
# --------------- Footer ---------------
st.caption(
    "Top-K is selected strictly from the prediction DataFrame. "
    "PDFs are optional; missing documents default to the tabular score."
)
